    ...
    public static void sort(List<Stock> list) {
        Collections.sort(list, new Comparator<Stock>() {
            public int compare(Stock stock1, Stock stock2) {
                int result = 0;
                if (result == 0 && stock1.containsKey("name") && stock2.containsKey("name"))
                {
                    result += ((String) stock1.get("name")).compareTo(((String) stock2.get("name")));
                }
                if(result == 0 && stock1.containsKey("date") && stock2.containsKey("date"))
                {
                    Date stock1Date = (Date) stock1.get("date");
                    stock1Date.setHours(0);
                    stock1Date.setMinutes(0);
                    stock1Date.setSeconds(0);
                    Date stock2Date = (Date) stock1.get("date");
                    stock2Date.setHours(0);
                    stock2Date.setMinutes(0);
                    stock2Date.setSeconds(0);
                    result += stock2Date.compareTo(stock1Date);
                }
                if(result == 0 && (stock1.containsKey("change") || (stock1.containsKey("open") && stock1.containsKey("last"))) && (stock2.containsKey("change") || (stock2.containsKey("open") && stock2.containsKey("last"))))
                {
                    ...
                    result += Double.compare(stock2Change, stock1Change);
                }
                return  result;
            }
        });
